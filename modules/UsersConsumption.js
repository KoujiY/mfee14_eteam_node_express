...
class UsersConsumption {
  ...
  async readUsersConsumption(uAcco) {
    if (uAcco) {
      const sql =
        "SELECT `orders`.`oId`,`iImg`,`oListName`,`sName`,`oQty`,`iPrice`,`oListPrice`,`oDate`FROM `items`JOIN `orders_list`ON `items`.`iId` = `orders_list`.`iId`JOIN `orders`ON `orders_list`.`oId` = `orders`.`oId`JOIN`specification`ON`items`.`iId`=`specification`.`iId`WHERE `orders`.uAcco = ?";
  ...
  async OrderByUsersConsumption(uAcco, value) {
    if (uAcco && value) {
      switch (value) {
        case "1":
          const sql1 =
            "SELECT `orders`.`oId`,`iImg`,`oListName`,`sName`,`oQty`,`iPrice`,`oListPrice`,`oDate`FROM `items`JOIN `orders_list`ON `items`.`iId` = `orders_list`.`iId`JOIN `orders`ON `orders_list`.`oId` = `orders`.`oId`JOIN`specification`ON`items`.`iId`=`specification`.`iId`WHERE `orders`.uAcco = ? ORDER BY `oDate` ASC";
        ...
        case "2":
          const sql2 =
            "SELECT `orders`.`oId`,`iImg`,`oListName`,`sName`,`oQty`,`iPrice`,`oListPrice`,`oDate`FROM `items`JOIN `orders_list`ON `items`.`iId` = `orders_list`.`iId`JOIN `orders`ON `orders_list`.`oId` = `orders`.`oId`JOIN`specification`ON`items`.`iId`=`specification`.`iId`WHERE `orders`.uAcco = ? ORDER BY `oDate` DESC";
